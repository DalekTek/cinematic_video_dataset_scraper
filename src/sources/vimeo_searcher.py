...
class VimeoSearcher(BaseRapidAPISearcher):
    """
    Класс для поиска видео на Vimeo.
    """

    def __init__(self, config: Config):
        super().__init__(config, 'vimeo')

    def _parse_results(self, data: dict, query: str) -> List[VideoMetadata]:
        """Парсит результаты ответа API для Vimeo."""
        videos = []
        for item in data.get('medias', []):
            try:
                ...
                videos.append(VideoMetadata(
                    title=item.get('title') or item.get('caption', {}).get('text') or "Vimeo Video",
                    url=url,
                ...
                self.logger.warning(f"Skipping invalid Vimeo item: {e}. Item: {item}")

        self.logger.info(f"Parsed {len(videos)} videos from Vimeo for query '{query}'.")
        return videos